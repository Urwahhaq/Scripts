...
HOSTS_FILE="/opt/downloads/install.list"
...
SPLUNK_FILE="/opt/downloads/splunkforwarder-6.5.2-67571ef4b87d-Linux-x86_64.tgz"
...
LOG_DIR="/opt/downloads"

# Set the user Splunk will run as
...
REMOTE_SCRIPT="$REMOTE_SCRIPT
  (cd \"$NEW_PARENT\" && sudo tar -zxf \"$DEST_FILE\") || fail \"could not untar /$DEST_FILE to $NEW_PARENT.\"
"

###   Reset File ownership
REMOTE_SCRIPT="$REMOTE_SCRIPT
  (sudo chown -R $SPLUNK_USER:$SPLUNK_USER \"$NEW_INSTANCE\") || fail \"could not reset ownership $NEW_INSTANCE.\"
"

###   start new instance.
REMOTE_SCRIPT="$REMOTE_SCRIPT
  sudo -u $SPLUNK_USER \"$NEW_INSTANCE/bin/splunk\" start --accept-license --answer-yes --auto-ports --no-prompt || \
...
if [ -n "$DEPLOY_SERV" ]; then
  REMOTE_SCRIPT="$REMOTE_SCRIPT
    sudo -u $SPLUNK_USER \"$NEW_INSTANCE/bin/splunk\" set deploy-poll \"$DEPLOY_SERV\" --accept-license --answer-yes \
      --auto-ports --no-prompt -auth admin:changeme || fail \"could not setup deployment client\"
  "
fi

###   setup bootstart.
REMOTE_SCRIPT="$REMOTE_SCRIPT
  sudo \"$NEW_INSTANCE/bin/splunk\" enable boot-start -user $SPLUNK_USER || \
    fail \"could not set bootstart\"
"
...
for DST in `cat "$HOSTS_FILE"`; do
  ...
  #echo "Checking pre-requisites for install on $DST..."
  #scp $SCP_PORT_ARG splunk_forwarder_check.sh $DST:/tmp/
  #ssh $SSH_PORT_ARG $DST /tmp/splunk_forwarder_check.sh
